...
- 已完成：总览、六类状态卡片、全面诊断、后台执行状态、问题/建议展示、分类详情、报告摘要、历史报告列表和 TXT / JSON / TXT+JSON 导出。
- 已复用：现有诊断对象、分类缓存、全面诊断、报告快照和报告导出函数；GUI 不复制诊断规则。
- 工具入口：网络、打印机、系统、磁盘、软件、电脑相关页面会始终列出原有工具；即使诊断结果全部正常，也不会隐藏修复或维护入口。
- 执行方式：点击工具会打开对应的原控制台动作，而不是重新实现一套高风险命令；原有的二次确认、UAC 管理员授权、执行输出和修复后复检都保留。
- 兼容模式：每个分类仍有“打开完整兼容工具箱”，可进入完整控制台菜单进行排障。
- 限制：第一版不把 SFC、DISM、winget、打印队列等长时间/交互式命令嵌入 WPF 进程；它们在独立控制台中运行，以避免 GUI 假死并保持原有交互逻辑。
...
诊断“正常”不代表这些工具不可用。GUI 分类详情页会始终提供下列入口，点击后启动相同的原控制台实现：

- 网络：技术详情、网络环境、网络诊断/复检、刷新 DNS、重新获取 IP、Winsock/TCP-IP 重置、重启网卡、一键网络修复。
- 打印机：打印机和队列查看、清理队列、重启 Spooler、一键修复、两个系统级深度修复脚本。
- 系统：SFC、DISM 检查/修复、Windows Update 服务检查和基础修复。
- 磁盘：磁盘空间、临时文件扫描/清理、清空回收站、磁盘健康检查。
- 软件：winget 搜索、安装、卸载、更新、已安装列表、常用软件。
- 电脑相关：电脑信息和硬件配置。
